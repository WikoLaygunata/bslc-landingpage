...
<template>
  <main class="min-h-screen overflow-x-clip bg-white">
    <section class="relative overflow-hidden bg-linear-to-b from-white via-slate-50 to-white px-4 py-14 sm:px-6 md:py-20">
      <div class="absolute right-0 top-0 h-72 w-72 translate-x-1/2 rounded-full bg-aqua/5 blur-3xl sm:h-96 sm:w-96 sm:translate-x-1/3 sm:bg-aqua/15"></div>
      <div class="absolute bottom-10 left-0 h-64 w-64 -translate-x-1/2 rounded-full bg-dark-green/5 blur-3xl sm:h-80 sm:w-80 sm:-translate-x-1/3 sm:bg-dark-green/10"></div>
      <div class="absolute left-1/2 top-36 hidden h-64 w-64 -translate-x-1/2 rounded-full bg-cyan/10 blur-3xl sm:block"></div>
      ...
      <div class="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        ...
        <form class="flex w-full gap-2 rounded-2xl border border-slate-200 bg-white/85 p-2 shadow-xl shadow-slate-200/60 backdrop-blur md:w-md" @submit.prevent="submitSearch">
          <input
            v-model="searchInput"
            type="search"
      ...
      >
        <div class="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-aqua/10 text-3xl font-bold text-aqua">
          ?
        </div>
        ...
        >
          <div class="flex w-full flex-col">
            <div class="relative h-36 overflow-hidden bg-dark-green sm:h-44">
              ...
              >
                <div class="absolute -left-8 -top-8 h-24 w-24 rounded-full bg-white/10 blur-2xl"></div>
                <div class="absolute -bottom-8 -right-8 h-24 w-24 rounded-full bg-cyan/20 blur-2xl"></div>
                <span class="text-lg font-bold text-white">{{ article.title }}</span>
              </div>
              <div class="absolute inset-x-0 bottom-0 h-20 bg-linear-to-t from-black/55 to-transparent opacity-0 transition duration-300 group-hover:opacity-100"></div>
              <div class="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 transition duration-300 group-hover:opacity-100">
                <span class="rounded-full border border-white/40 bg-white/20 px-4 py-2 text-xs font-bold text-white backdrop-blur sm:text-sm">
                  Read Article
                </span>
              </div>
            </div>
